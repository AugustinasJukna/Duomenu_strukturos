...
public class LinkedList<E extends Comparable<E>>
    ...
    public boolean add(int k, E e) {
        ...
        if (k < 0 || k >= size) {
            ...
            for (Node w = first; w != null; w = w.next) {
                if (index == k && w != first) {
                    Node previous = this.first.findNode(index - 1);
                    previous.next = new Node(e, w);
                    size++;
                    return true;
                }

                else if (index == k && w == first) {
                    first = new Node(e, w);
                    size++;
                    return true;
                }
    ...
    public E set(int k, E e) {
        Node toChange = this.first.findNode(k);
        E holder = (E) toChange.element;
        toChange.element = e;
        return holder;
    }
    ...
    public E remove(int k) {
        ...
        if (this.first != null)
            toRemove = this.first.findNode(k);
            if (toRemove == null)
                return null;
            ...
            if (toRemove == this.first) {
                ...
                Node previous = this.first.findNode(k - 1);
    ...
    class Iterator implements java.util.Iterator<E> {
        ...
        @Override
        public void remove() {
            if (iterPosition == first.next && iterPrevious == first) {
                first = iterPosition;
            }
            else {
                Node previous;
                for (previous = first; previous.next != iterPrevious; previous = previous.next);
                if (iterPrevious == last) {
                    previous.next = null;
                    last = previous;
                }
                else {
